...
syms s
PIcc = GFMInit.KPC + GFMInit.KIC/s;
fvf = GFMInit.KVF/(GFMInit.TVF*s+1);
PIvc = GFMInit.KPV + GFMInit.KIV/s;
GI = PIcc/(PIcc+s*GFMInit.LF/W0);
YVF = (1-fvf)/(PIcc + s*GFMInit.LF/W0);
Y0 = (YVF + GI*PIvc+GI*s*GFMInit.CF/W0)/(1-GI);
Y0 = inf;
Zmtheta = -GFMInit.Vd0*(GFMInit.Idref0/Y0+GFMInit.Vd0)/((GFMInit.J*s^2+GFMInit.D*s)/W0)*[sin(GFMInit.theta0)*cos(GFMInit.theta0) sin(GFMInit.theta0)^2;-cos(GFMInit.theta0)^2 -sin(GFMInit.theta0)*cos(GFMInit.theta0)];
Y0 = 1/(0.0015+0*0.025*s/W0)*SGFM;
Zxygfm = diag([inv(Y0) inv(Y0)])+Zmtheta+(GFMInit.LG)*[s/W0 -1;1 s/W0]+GFMInit.RG*eye(2);
Q = 1/resultsac.branch{1,4};
tao = 0;
gama = 1/((s+tao)^2/W0+W0)*[s+tao W0;-W0 s+tao];
...
mdl='OneGFM_V2';
open_system(mdl);
io(1)=linio('OneGFM_V2/GFM1',1);
io(2)=linio('OneGFM_V2/transmission network',1,'output');
linsys1=linearize(mdl,io);
[EIG_Complex] = eig(linsys1);
EIG_PLL = EIG_Complex(real(EIG_Complex)>0)