...
classdef GridFormingVSI < SimplusGT.Class.ModelAdvance
    ...
    methods(Static)
        ...
        function [State,Input,Output] = SignalList(obj)
            if obj.ApparatusType == 20
         	State  = {'i_ld','i_lq','i_ld_i','i_lq_i','v_od','v_oq','v_od_i','v_oq_i','i_od','i_oq','v_d_ref','w','theta'};
            else
                State  = {'i_ld','i_lq','i_ld_i','i_lq_i','v_od','v_oq','v_od_i','v_oq_i','i_od','i_oq','v_d_ref','w','theta','v_d_i','v_q_i'};
%                 State  = {'i_ld','i_lq','i_ld_i','i_lq_i','v_od','v_oq','v_od_i','v_oq_i','i_od','i_oq','v_d_ref','w','theta','v_d_i','v_q_i'};
            end
            Input  = {'v_d','v_q','P0'};
...
%                 Rc = 0;
            ...
            %　由于此处有网侧串联电感／滤波电感，这里的v_oq_r不等于０，需要处理一下，坐标轴要旋转一下
            if obj.ApparatusType == 21 || obj.ApparatusType == 22
                v_odq_r = v_od_r+1i*v_oq_r;
                arg_vo = angle(v_odq_r);
%                 abs_vo = abs(v_odq_r);
                xi = xi + arg_vo;
                v_gdq = V*exp(-1i*arg_vo);
                ...
                i_odq = i_odq*exp(-1i*arg_vo);
                i_ldq = i_ldq*exp(-1i*arg_vo);
                v_odq = v_odq*exp(-1i*arg_vo);
                e_dq  = e_dq*exp(-1i*arg_vo);
                i_ld = real(i_ldq);
                i_lq = imag(i_ldq);
                i_ld_i = real(e_dq);
                i_lq_i = imag(e_dq);
                ...
                i_od = real(i_odq);
                i_oq = imag(i_odq);
                % -i_ld = v_od_i - w*Cf*v_oq -i_od
                % -i_lq = v_oq_i + w*Cf*v_od -i_oq
                v_od_i = w*Cf*v_oq + i_od - i_ld;
                v_oq_i = -w*Cf*v_od + i_oq - i_lq;
                theta = xi;
                
                v_d_i = v_od;
                v_q_i = v_oq;
                
                i_ld_i = real(e_dq) - v_d_i + w*Lf*(-i_lq) - Rf*(-i_ld); % 电流是向里的
                i_lq_i = imag(e_dq) - v_q_i - w*Lf*(-i_ld) + Rf*(-i_lq); 
            ...
            if obj.ApparatusType == 20
                ...
                x_e = [i_ld; i_lq; i_ld_i; i_lq_i; v_od; v_oq; v_od_i; v_oq_i; i_od; i_oq; v_od_r; w; theta;v_d_i;v_q_i];
            end
            u_e = [v_gd; v_gq; P0];
%             xi	= obj.PowerFlow(4);
        end
...
%                 Rc = 0;
                J = obj.Para(12);
                J = J; % Jpu=J/Pb=[1/2*J*w0^2/Pb]*2/w0^2, [MWs/MW]
                D = xDw;   % Dpu=dTpu/dw=dPpu/dw/w0=[dP%/dw%]/w0^2, [%/%]
            end
            if obj.ApparatusType == 22
                SB = obj.Para(13);
                xwLf = 0.05/SB;
...
%                 Rc = 0;
                J = obj.Para(12);
                J = J*SB; % Jpu=J/Pb=[1/2*J*w0^2/Pb]*2/w0^2, [MWs/MW]
                D = xDw*SB;   % Dpu=dTpu/dw=dPpu/dw/w0=[dP%/dw%]/w0^2, [%/%]
            end
            % Update paramters
            Lf = xwLf/W0;
            Cf = xwCf/W0;
            ...
            % ### Call state equation: dx/dt = f(x,u)
                ...
                if obj.ApparatusType == 21 || obj.ApparatusType == 22
                        dw = (P0 - p - D*(w-W0)/W0)/(J/W0); % J 太小是否会导致数值问题，需要理论推导
                end
                ...
                %
                if obj.ApparatusType == 21 || obj.ApparatusType == 22
                    i_ld_r = -(error_v_od*kp_v_odq + v_od_i + (-i_od) - w*Cf*v_oq);
                    i_lq_r = -(error_v_oq*kp_v_odq + v_oq_i + (-i_oq) + w*Cf*v_od);
                end
                ...
                if obj.ApparatusType == 21 || obj.ApparatusType == 22
                    v_d_i = x(14);
                    v_q_i = x(15);
                    dv_d_i = (v_od - v_d_i)/0.02;
                    dv_q_i = (v_oq - v_q_i)/0.02;
                    
                    e_d = v_d_i + e_d - w*Lf*(-i_lq) + Rf*(-i_ld);
                    e_q = v_q_i + e_q + w*Lf*(-i_ld) + Rf*(-i_lq);
...
%                 end
                ...
                if obj.ApparatusType == 20
                    ...
                    f_xu = [di_ld; di_lq; di_ld_i; di_lq_i; dv_od; dv_oq; dv_od_i; dv_oq_i; di_od; di_oq; dv_od_r; dw; dtheta;dv_d_i;dv_q_i];
                end
                Output = f_xu;
