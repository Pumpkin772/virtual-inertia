...
tic;
Init_Parameters;
J = [4.5 11.6 11.6 4.5 11.6 1.9 1.7 11.6 11.6 11.6];
...
for Ind_fau = 1:num_fau
    [~,Inerlocation]=ismember(setdiff(NodeS,fau(Ind_fau)),Node_re);%返回无扰动的待配置储能节点与同步机节点在降阶后节点集合Node_re的编号索引
    [~,Distlocation]=ismember(fau(Ind_fau),Node_re);%返回扰动节点在降阶后节点集合Node_re的编号索引
    ...
    [~,DistlocatES]=ismember(fau(Ind_fau),NodeESCandidate);%分布在
    ...
    Constraints = [Constraints;-2*pi*(Sa_c{Ind_fau,1}(ESInterCanlocation,1))<=Del_c{Ind_fau,1}(ESInterCanlocation,1)...
        <=0*pi*(Sa_c{Ind_fau,1}(ESInterCanlocation,1))]; % 大M法，储能没有达到饱和时，相角为0；达到饱和时，功率为最大输出功率
    Constraints = [Constraints;Pes_g_c{Ind_fau,1}==L(ESCanlocation,:)*Del{Ind_fau,1}];% 储能输出功率和网络之间的关系
    % 发电机节点频率变化率约束
    Constraints = [Constraints;Psg{Ind_fau,1}==L(Sglocation,:)*Del{Ind_fau,1}];
    Constraints = [Constraints;RoCoF_sg{Ind_fau,1}==(-L(Sglocation,:)*Del{Ind_fau,1})./J'*FreqBase];% 发电机输出功率除以惯性
...
save('TENSG_Saturation0305/sixGFMconfig');
%% 
% 绘制频率变化率
figure(1)
